...
function mergeD100Results(draginfo)
  ...
  if customData then
	  ...
	  if customData.ctNodeStunRR then
		local staminarr = 0;
		for i,v in ipairs(dielist) do
			staminarr = staminarr + v.result;
		end
		staminarr = staminarr + draginfo.getNumberData();
		customData.ctNodeStunRR.getChild("stunroll").setValue(staminarr);
	  end
...
function processRMCDieRoll(dieList, draginfo, custData)
  ...
  if custData.ctNodeStunRR then
		custData.ctNodeStunRR.createChild("stunroll","number").setValue(modroll);
  end
	  ----------- Put code in here to store result of rr
  -- Unless the roll is an attack type, the size doesn't matter
  
  if custData.size then
	if modroll > 0 then
		if custData.size == 1 then
			table.insert(custData.modifiers, {description="Size: Tiny", value=-20});
			custData.max_roll = 80;
		elseif custData.size == 2 then
			table.insert(custData.modifiers, {description="Size: Small", value=-10});
			custData.max_roll = 90;
		elseif custData.size == 3 then
			table.insert(custData.modifiers, {description="Size: Medium", value=0});
			custData.max_roll = 100;
		elseif custData.size == 4 then
			table.insert(custData.modifiers, {description="Size: Large", value=10});
			custData.max_roll = 110;
		elseif custData.size == 5 then
			table.insert(custData.modifiers, {description="Size: Huge", value=20});
			custData.max_roll = 120;
		end
	end
  end
  if custData.unmodifiedRoll > 99 then
    custData.max_roll = 120;
  end
