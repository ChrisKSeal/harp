	...
	if node.getChild("combat") then
		...
		if cmbtnode.getChild("defences") then
			...
			for i, itm in pairs(defnode.getChildren()) do
				...
				local ppadd = itm.createChild("pp_adder","number").getValue();
				if idflg == 1  and ppadd > 0 then
					...
					if not flg then
						...
						newentry.pp_adder.setValue(ppadd);
					end
				end
			end
		end
...
function isLinked(itemlink, charnode)
	...
	local ppnode = mgnode.createChild("pp_adders");
