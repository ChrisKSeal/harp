...
function onInit()
  ...
  if nodelink and nodelink[1] then
	nodename = nodelink[1]
	nodecontrol = window[nodename];
  end
...
function selectitem(opt)
  ...
  selecteditem = opt;
  if nodecontrol.getValue() ~= getValue() and getValue() ~= "" then
	nodecontrol.setValue(getValue());
  end
  if targetcontrol.getValue()~=getText() and getText() ~= "" then
    targetcontrol.setValue(getText());
  end
end
